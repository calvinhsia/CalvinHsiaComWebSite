...
window.enhanceWordamentDesktopDrag = function () {
    ...
    window.wordamentMouseHandlers = {
        ...
        mouseUp: function (e) {
            // CRITICAL FIX: Always handle mouseup if we have a valid path, regardless of isDragging flag
            // This prevents the intermittent issue where isDragging gets corrupted
            const hasValidPath = window.wordamentDragState &&
                window.wordamentDragState.dragPath &&
                window.wordamentDragState.dragPath.length > 0;

            if (!window.wordamentDragState.isDragging && !hasValidPath) {
                debugLog('Mouse up ignored - no active drag and no valid path');
                return;
            }

            // Log state for debugging
            if (hasValidPath && !window.wordamentDragState.isDragging) {
                debugWarn('RACE CONDITION DETECTED: mouseUp called with valid path but isDragging=false. Recovering...');
            }

            e.preventDefault();
            e.stopPropagation();

            debugLog('JavaScript: Desktop drag ended. Path:', window.wordamentDragState.dragPath, 'isDragging was:', window.wordamentDragState.isDragging);

            // Notify Blazor about drag end (only if we have a path)
            if (hasValidPath) {
        ...
        mouseLeave: function (e) {
            // More defensive: handle mouseleave even if isDragging is false but we have a path
            const hasValidPath = window.wordamentDragState &&
                window.wordamentDragState.dragPath &&
                window.wordamentDragState.dragPath.length > 0;

            if (window.wordamentDragState.isDragging || hasValidPath) {
                debugLog('JavaScript: Mouse left grid during drag - triggering mouseUp');
