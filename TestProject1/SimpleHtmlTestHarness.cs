...
namespace TestProject1
{
    ...
    public class SimpleHtmlTestHarness
    {
        ...
        private static string? _tempHtmlPath;

        /// <summary>
        /// Opens a browser in incognito mode with the specified HTML file
        /// </summary>
        private static Process? OpenBrowserIncognito(string htmlPath)
        {
            try
            {
                // Try Chrome first (most common)
                var chromeArgs = $"--incognito \"{htmlPath}\"";
                var chromePaths = new[]
                {
                ...
                foreach (var chromePath in chromePaths)
                {
                    if (File.Exists(chromePath))
                    {
                        ...
                        Console.WriteLine("Opened in Chrome (Incognito mode)");
                        return process;
                    }
                }

                // Try Edge
                var edgeArgs = $"-inprivate \"{htmlPath}\"";
                var edgePaths = new[]
                {
                ...
                foreach (var edgePath in edgePaths)
                {
                    if (File.Exists(edgePath))
                    {
                        ...
                        Console.WriteLine("Opened in Edge (InPrivate mode)");
                        return process;
                    }
                }

                // Fallback: Use default browser (not incognito)
                Console.WriteLine("Chrome/Edge not found, opening in default browser (not incognito)...");
                Process.Start(new ProcessStartInfo
                {
                    FileName = htmlPath,
                    UseShellExecute = true
                });
                return null; // Can't track default browser process
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Waits for user to close the browser window
        /// </summary>
        private static async Task WaitForBrowserClose(Process? browserProcess, string htmlPath)
        {
            if (browserProcess != null && !browserProcess.HasExited)
            {
                Console.WriteLine("\n🎮 Browser is open. Close the browser window when done.");
                Console.WriteLine("The test will wait until the browser is closed...\n");

                await Task.Run(() => browserProcess.WaitForExit());
                Console.WriteLine("✅ Browser closed.");
            }
            else
            {
                Console.WriteLine("\n⚠️  Could not track browser process.");
                Console.WriteLine($"📄 HTML file: {htmlPath}");
                Console.WriteLine("Press Enter when you're done with the browser...");
                Console.ReadLine();
            }
        }
        ...
        [TestMethod]
        [TestCategory("Manual")]
        public async Task CreateInteractiveHtmlTestHarness()
        {
...
<body>
    <div class=""test-harness-container"">
      ...
      <p><strong>🔒 You're in Incognito/Private mode</strong> - No cache, cookies, or history saved.</p>
 ...
 cell.classList.remove('selected');
  ...
  log('Tips:');
    ...
    log('- Use browser DevTools (F12) to debug and inspect');
        log('- Close this window when done to end the test');
    </script>
</body>
</html>";
...
<body>
    <div class=""sidebar"">
      <div class=""incognito-badge"">🔒 Incognito Mode</div>
        <h2>🧪 Test Controls</h2>
 ...
 <select id=""pageSelector"" onchange=""navigateToPage()"">
   ...
   <option value=""/cartoon"">Cartoon</option>
    ...
    <option value=""/fish"">Fish vs Sharks</option>
   </select>
...
<button onclick=""injectCustomJs()"">💉 Inject JS</button>
  ...
  <button onclick=""takeScreenshot()"">📸 Screenshot Info</button>
        <button onclick=""getPageInfo()"">ℹ️ Page Info</button>
    </div>
...
<iframe id=""blazorFrame"" src=""http://localhost:5000""></iframe>
   ...
   <div class=""log-section"" id=""logSection"">
      <div class=""log-entry info"">Test harness initialized. Start your Blazor WASM app with: dotnet run</div>
       <div class=""log-entry warning"">Close this window when done to end the test.</div>
        </div>
    </div>
